...
test_that("spdf_jpn_pref", {
  expect_s4_class(df.pref, "SpatialPolygonsDataFrame")
  expect_equal(spdf_jpn_pref(code = 33, district = FALSE)@data$pref_name, stringi::stri_unescape_unicode("\\u5ca1\\u5c71\\u770c"))
  expect_error(spdf_jpn_pref(admin_name = "\\u5ca1\\u5c71\\u770c", code = 12), info = "provide multiple arguments")
})

test_that("spdf_jpn_cities", {
  expect_s4_class(df.city, "SpatialPolygonsDataFrame")
  expect_named(df.city@data,
               c("pref_name", "city_name_", "city_name", "city_name_full", "city_code"))
...
test_that("Collect administration offices data", {
  expect_s4_class(df.admins, "SpatialPointsDataFrame")
  expect_equal(dim(df.admins@data), c(65, 6))
  expect_named(df.admins@data,
